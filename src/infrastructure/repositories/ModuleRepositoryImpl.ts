...
export class ModuleRepositoryImpl implements ModuleRepository {
    ...
    async getAllModules(): Promise<IModule[]> {
        const [rows] = await ModuleRepositoryImpl.pool.execute("SELECT * FROM modulos");
        return rows as IModule[];
    }

    async getModuleById(id: number): Promise<IModule | null> {
        const [rows] = await ModuleRepositoryImpl.pool.execute("SELECT * FROM modulos WHERE id = ?", [id]);
        return (rows as IModule[])[0] || null;
    }

    async createModule(module: Omit<IModule, "id">): Promise<IModule> {
        const [rows] = await ModuleRepositoryImpl.pool.execute<ResultSetHeader>("INSERT INTO modulos (name) VALUES (?)", [module.name]);
        const { insertId } = rows;
        return { ...module, id: insertId };
    }

    async updateModule(module: IModule): Promise<IModule> {
        await ModuleRepositoryImpl.pool.execute("UPDATE modulos SET ? WHERE id = ?", [module, module.id]);
        return module;
    }

    async deleteModule(id: number): Promise<void> {
        await ModuleRepositoryImpl.pool.execute("DELETE FROM modulos WHERE id = ?", [id]);
    }   
