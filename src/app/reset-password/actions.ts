...
export async function requestPasswordResetByEmail(email: string) {
  ...
  await sendEmail(
    ...
    resetPasswordTemplate(resetLink, email)
  );
...
export async function updatePassword(idUser: number, newPassword: string) {
  const hashedPassword = await bcrypt.hash(newPassword, 10);
  await resetPassword(idUser, hashedPassword);
